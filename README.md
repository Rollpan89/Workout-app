...
| **Introduktion** vid första start (4 steg, språk först), **ErrorBoundary** med lugn felsida, **opt-in felrapporter** (av som standard) (v4) | ✅ |
...
- `customWorkout.ts` – **utkastmodellen för egna pass** (`CustomWorkoutDraft`): en lista av övningar med set, reps-eller-sekunder, vila och en sektion (`warmup` / `main` / `stretch`; saknas = träning). `rounds` (1–10) upprepar bara träningen. `compileDraft()` gör om utkastet till en vanlig `Workout` (uppvärmning en gång, träning med varv, stretch en gång; ett ensamt träningsblock heter fortfarande Cirkel / Ditt pass). Planner, motor, coach och summering behöver inte veta att passet är egenbyggt. `draftFromWorkout()` behåller uppvärmning och stretch som egna sektioner och varven när alla träningsblock har samma antal; blandade varv expanderas så arbetsmängden stämmer. `validateDraft()` + `DRAFT_LIMITS` sätter gränserna.
...
| `WorkoutBuilderScreen` + `ExercisePicker` + `DraftExerciseRow` | `/builder/[id]` | Byggaren: namn, färg, mål, nivå, tre sektioner (**uppvärmning**, **träning**, **stretch**) med steppers (set / reps eller sekunder / vila), reps↔tid, ordning inom sektionen, info-ark, **varv** (1–10, bara träningen), vila mellan övningar, validering. `id = new` (tomt), `new?from=<id>` (kopia), `<eget id>` (redigera) eller `new?override=<inbyggt id>` (**admin**: redigera ett förinställt pass på plats) |
...
| `core/__tests__/customWorkout.test.ts` | Utkast: validering, defaults per kategori, färgrotation, kompilering till körbar `Workout`, **varv bara på träningen** (uppvärmning/stretch en gång, Cirkel/Ditt pass när det bara finns träning), kopiering behåller sektioner, repository-CRUD, **historik-tak (365) + checkpoint-rundtur** |
| `core/__tests__/shareCode.test.ts` | Delningskoden: rundtur, kod inbäddad i en mening, klämning av varje tal, okända övningar hoppas över och rapporteras, sektion (`c`) för uppvärmning/stretch, samtliga felfall (`empty`/`malformed`/`unsupportedVersion`/`noName`/`noExercises`) |
...
- **Admin-läget använder samma tre sektioner som egna pass.** Uppvärmning och stretch körs en gång; varv (1–10) upprepar bara träningen. Blandade träningsblock (cirkel + finisher med olika varv) expanderas så arbetsmängden inte ändras. Redigeringen ligger lokalt på enheten (`workoutOverrides`) och försvinner inte när appen uppdateras, men den följer inte med till en annan enhet och slår inte igenom på nya inbyggda pass.
...
- **Egna pass har tre sektioner:** uppvärmning, träning och stretch. Bara träningen upprepas av varv (1–10). Modellen kompileras till samma `Workout`-form som de färdiga programmen, så motorn är orörd.
